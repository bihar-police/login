...
export const NewFIREntryModal: React.FC<NewFIREntryModalProps> = ({
  ...
  const handleAddAccused = () => {
    ...
    if (accusedNames.includes(accusedInput.trim())) {
      alert('Accused already in list!');
      return;
    }
    setAccusedNames((prev) => {
      const next = [...prev, accusedInput.trim()];
      setAccusedCount((c) => Math.max(c, next.length));
      return next;
    });
    setAccusedInput('');
  };

  const handleRemoveAccused = (nameToRemove: string) => {
    setAccusedNames((prev) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      accusedCount: Number(accusedCount) || accusedNames.length,
      accusedList: accusedNames.map((name, i) => ({
        id: `acc-${Date.now()}-${i}-${Math.random().toString(36).substring(2, 5)}`,
        name,
        status: 'Enquiry',
      })),
    });
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl border border-slate-200 dark:border-slate-700/80 space-y-3">
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-0.5">
                Total Accused added: <strong className="text-indigo-600 dark:text-indigo-400">{accusedNames.length}</strong>. (If name is unknown, you can add names later during supervision)
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-2 border-b border-slate-200 dark:border-slate-700/50">
              <div>
                <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">
                  Total Number of Accused <span className="text-rose-500">*</span>
                </label>
                <input
                  type="number"
                  min={accusedNames.length}
                  value={accusedCount || ''}
                  onChange={(e) => setAccusedCount(Math.max(accusedNames.length, Number(e.target.value) || 0))}
                  placeholder="e.g. 5"
                  required
                  className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl p-2 text-xs font-semibold text-slate-900 dark:text-white"
                />
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Enter accused name (e.g. Ramesh Mahto)"
                className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl p-2 text-xs font-semibold text-slate-900 dark:text-white"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleAddAccused();
            ...
            {accusedNames.length > 0 ? (
              ...
              <p className="text-[10px] text-slate-400 italic">No accused added yet. (Case registered against unknown person/persons)</p>
            )}
          </div>
